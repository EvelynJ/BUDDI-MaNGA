...
pro create_BUDDI_start_file_poly2,dir,cube,plate_ifu,SPA,Manga_list,Pymorph_list,loop,element,wave_log,ONECOMP=onecomp,TWOCOMP=twocomp

close,01
if keyword_set(TWOCOMP) then openw,01,dir+'BUDDI_twocomp_'+plate_ifu+'_poly2.txt' $
  else if keyword_set(ONECOMP) then openw,01,dir+'BUDDI_onecomp_'+plate_ifu+'_poly2.txt' $
  else openw,01,dir+'BUDDI_input_onecomp_'+plate_ifu+'_poly2.txt'
...
printf,01,'A07)   binned_images_poly2/                  # [string] directory for binned images (binned_dir)'
printf,01,'A08)   image_slices_poly2/                   # [string] directory for image slices (slices_dir)'
printf,01,'A09)   decomposed_data_poly2/                # [string] directory for image slices (decomp_dir)'
...
printf,01,'B00)   /raid/ejohnston/Galfitm/galfitm-1.4.4-linux-x86_64   # [string] directory and executable for GalfitM (galfitm)'
printf,01,' '
printf,01,' '
...
printf,01,'E06)  n            # [y/n] decompose_median_image'
...
;  printf,01,'F12)    '+strtrim(((Re_kpc[element]/Scale[loop])/pix_scale)*1.678,2)+'      # Re for disk'
  ...
  printf,01,'F16)    2        # polynomial order for variation in Re (negative for free)'
  printf,01,'F17)    2       # polynomial order for variation in n (negative for free)'
