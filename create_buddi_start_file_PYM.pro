...
;pro create_BUDDI_start_file,dir,cube,plate_ifu,Manga_list,Simard_SS,Simard_BD,loop,element,wave_log,ONECOMP=onecomp,TWOCOMP=twocomp
pro create_buddi_start_file_PYM,dir,cube,plate_ifu,SPA,Manga_list,Pymorph_list,loop,element,wave_log,ONECOMP=onecomp,TWOCOMP=twocomp

close,01
if keyword_set(TWOCOMP) then openw,01,dir+'BUDDI_twocomp_'+plate_ifu+'_PYM.txt' $
  else if keyword_set(ONECOMP) then openw,01,dir+'BUDDI_onecomp_'+plate_ifu+'_PYM.txt' $
  else openw,01,dir+'BUDDI_input_onecomp_'+plate_ifu+'_PYM.txt'
...
if keyword_set(TWOCOMP) then decomp_dir='IFU_decomp_2comp_PYM/' $
else if keyword_set(ONECOMP) then decomp_dir='IFU_decomp_1comp_PYM/' $
else decomp_dir='IFU_decomp_1comp_PYM/'
...
printf,01,'A11)   /raid/ejohnston/GitHub/BUDDI/miles_models/ # [string] directory for stellar library (stellib_dir)'
...
printf,01,'B00)   /raid/ejohnston/Galfitm/galfitm-1.2.1-linux-x86_64   # [string] directory and executable for GalfitM (galfitm)'
printf,01,' '
printf,01,' '
...
printf,01,'E04)  n            # [y/n] correct_kinematics'
...
;  PA=Simard_SS.phi
  ...
  PA_temp=Pymorph_list.PA_S
  PA_x=(90-PA_temp[element])-SPA[element]
  ;if PA ge 0 then PA-=90 else PA+=90
;  if PA ge 0 and PA lt 90 then PA=90-PA 
;  if PA ge 90 then PA=-(PA-90) 
;  if PA lt 0 and PA gt -90 then PA=90-PA
;  if PA le -90 then PA=-PA-90
  if PA_x ge 0 and PA_x lt 90 then PA=90-PA_x 
  if PA_x ge 90 then PA=180-PA_x
  if PA_x lt 0 and PA_x gt -90 then PA=-(90+PA_x)
  if PA_x le -90 then PA=abs(PA_x)-90;PA_x+90
  ;print,PA_temp[element], PA_x,PA,SPA[element]
...
;  printf,01,'F12)    '+strtrim(((Re_kpc[element]/Scale[loop])/pix_scale)*1.678,2)+'      # Re for disk'
  ...
  printf,01,'F16)    0        # polynomial order for variation in Re (negative for free)'
  printf,01,'F17)    0       # polynomial order for variation in n (negative for free)'
  printf,01,'F18)    0        # polynomial order for variation in q (negative for free)'
  printf,01,'F19)    0        # polynomial order for variation in pa (negative for free)'
  printf,01,' '
  printf,01,' '
  ...
  printf,01,'F26)    0        # polynomial order for variation in Re (negative for free)'
  printf,01,'F27)    0       # polynomial order for variation in n (negative for free)'
  printf,01,'F28)    0        # polynomial order for variation in q (negative for free)'
  printf,01,'F29)    0        # polynomial order for variation in pa (negative for free)'
  printf,01,' '
  printf,01,' '
endif
...
printf,01,'F36)    0        # polynomial order for variation in Re (negative for free)'
printf,01,'F37)    0        # polynomial order for variation in n (negative for free)'
printf,01,'F38)    0        # polynomial order for variation in q (negative for free)'
printf,01,'F39)    0        # polynomial order for variation in pa (negative for free)'
printf,01,' '
printf,01,' '
...
printf,01,'F46)    0        # polynomial order for variation in Re (negative for free)'
printf,01,'F47)    0       # polynomial order for variation in n (negative for free)'
printf,01,'F48)    0        # polynomial order for variation in q (negative for free)'
printf,01,'F49)    0        # polynomial order for variation in pa (negative for free)'
printf,01,' '
printf,01,' '
