...
for n=0,n_elements(in_prog)-1,1 do begin
  ...
  if file_test(root_fits+in_prog[n]+'/IFU_decomp_1comp_PYM/decomposed_data/',/DIRECTORY) eq 1 then val_ss = 1 else val_ss = -1
  if file_test(root_fits+in_prog[n]+'/IFU_decomp_2comp_PYM/decomposed_data/',/DIRECTORY) eq 1 then val_se = 1 else val_se = -1
  print,n,' ',in_prog[n];,val_ss,val_se
  
  

  if val_ss ne -1 then begin
    res=read_sersic_results_2comp(root_fits+in_prog[n]+'/IFU_decomp_1comp_PYM/binned_images/imgblock.fits',12,bd=0)
  ...
  if val_se ne -1 then begin
    res=read_sersic_results_2comp(root_fits+in_prog[n]+'/IFU_decomp_2comp_PYM/binned_images/imgblock.fits',12,bd=1)
...
MWRFITS, catalog, root_output+'BUDDI_MaNGA_cat_final_PYM.fits'
